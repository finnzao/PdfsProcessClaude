...
Bonus se o sinal aparece nos primeiros 400 chars (cabecalho).
Score minimo por tipo evita falso positivo em citacoes.

Suporta sinais como str (substring) ou re.Pattern (regex).
"""
...
TIPOS_PECAS = [
    ...
    ("RECURSO", [
        ...
        (re.compile(r"\bapela[çc][aã]o\b", re.I), 1),  # peso baixo: muito citado
    ], 8),
...
# Pecas onde a cautelar pode ser fixada
...
def classificar_peca_com_score(texto: str, janela: int = 3000) -> tuple[str, int]:
    """
    Classifica peca por scoring ponderado.

    Retorna (tipo, score). Score e diagnostico — 0 quando nenhum tipo bateu
    o minimo, valor positivo quando um tipo venceu.
    """
    ...
    for tipo, sinais, minimo in TIPOS_PECAS:
        ...
        for sinal, peso in sinais:
            if _testar_sinal(sinal, texto_amostra, texto_lower):
                score += peso
                # Bonus por aparecer no cabecalho
                if isinstance(sinal, re.Pattern):
                    if sinal.search(inicio):
                        score += peso
                else:
                    if sinal.lower() in inicio_lower:
                        score += peso
...
def classificar_peca(texto: str) -> str:
    """Versao que retorna so o tipo."""
    return classificar_peca_com_score(texto)[0]
